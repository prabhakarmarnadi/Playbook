#!/usr/bin/env python3
"""Run the minimal pipeline with sanity-check → full-run workflow.

Usage:
    # Sanity check on 10 docs first:
    python scripts/run_full_pipeline.py --sanity

    # Full run on all 510 docs:
    python scripts/run_full_pipeline.py --full

    # Export existing DB to shareable JSON bundle:
    python scripts/run_full_pipeline.py --export-only --db data/v11_sanity.duckdb

    # Resume: skip stages already complete
    python scripts/run_full_pipeline.py --full --resume
"""
...
# ── Constants ─────────────────────────────────────────────────────────────────
PDF_DIR = "/home/azureuser/clustering-v2/legacy_stack/apr-fields-processor/cuad_clustering_output/full_contract_txt"
RUN_TAG = "v11"
...
def build_config(mode: str, paths: dict) -> MinimalPipelineConfig:
    """Build a high-performance pipeline config."""
    ...
    return MinimalPipelineConfig(
        ...
        max_docs=10 if is_sanity else None,
...
def export_db_to_json(db_path: str, output_dir: str) -> Path:
    ...
    Creates:
        {output_dir}/
            export_manifest.json    — summary + metadata
            domains.json
            clusters.json           — with keywords, descriptions
            field_definitions.json  — field schemas per cluster
            extractions.json        — all extracted values
            agreements.json         — doc metadata (no raw text, too large)
            clauses_summary.json    — clause headers + IDs (not full text)
            intent_types.json       — intent registry
            clause_intents.json     — intent instances
            cluster_assignments.json — chunk→cluster mapping
    """
    import duckdb
    ...
    def _query_json(sql: str, filename: str, transform=None):
        """Run query, optionally transform rows, write to JSON."""
...
def export_parquet_backup(db_path: str) -> Path:
    """Also create a Parquet backup for efficient sharing/loading."""
...
async def run_pipeline(mode: str) -> dict:
    ...
    logger.info(f"{'=' * 80}")
    logger.info(f" PIPELINE RUN: {RUN_TAG}_{mode}")
    logger.info(f"{'=' * 80}")
    logger.info(f"  Mode:     {mode}")
    logger.info(f"  Max docs: {config.max_docs or 'ALL (510)'}")
...
def main():
    parser = argparse.ArgumentParser(description=f"Run minimal pipeline ({RUN_TAG})")
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--sanity", action="store_true", help="Run on 10 docs as sanity check")
    group.add_argument("--full", action="store_true", help="Run on all 510 docs")
    group.add_argument("--export-only", action="store_true", help="Export existing DB to JSON (no pipeline run)")
