#!/usr/bin/env python3
"""Full-scale run v4: Late interaction + wide scope (config bug fixed) + lazy persist.

Changes from v3:
  - Fixed shared_rag_config to pass use_late_interaction and widen_retrieval_scope
  - ColBERT MaxSim and agreement-wide retrieval should now actually activate
  - Lazy-persist continues from v3

Expected impact:
  - More fields getting extractions (fewer retrieval_miss drops)
  - Better extraction quality from per-token matching
  - Wider chunk visibility per agreement

Usage:
    nohup python scripts/run_full_v4.py > data/full_v4.log 2>&1 &
"""
...
REPORT_PATH = DATA_DIR / "full_v4_report.txt"
RESULT_PATH = DATA_DIR / "full_v4_result.json"
...
def build_config() -> MinimalPipelineConfig:
    return MinimalPipelineConfig(
        ...
        # ── retriever upgrades (NOW WIRED THROUGH shared_rag_config) ───
...
def generate_report(result: dict, wall_time: float):
    ...
    p("=" * 80)
    p(f"  FULL-V4 RUN — REPORT (ColBERT + wide scope + lazy persist — config fix)")
...
async def main():
    ...
    print(f"Starting full-v4 run at {datetime.now().strftime('%H:%M:%S')}")
    ...
    print(f"  (shared_rag_config bug FIXED — these flags will propagate)")
    print()
