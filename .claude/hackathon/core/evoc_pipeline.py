...
# NOTE: These are defaults. Use adaptive_param_ranges() for dataset-aware ranges.
EVOC_PARAM_RANGES = {
    "base_min_cluster_size": (3, 25),
    "noise_level": (0.15, 0.40),
    "n_neighbors": (5, 30),
    "min_samples": (1, 8),
}


def adaptive_param_ranges(n_items: int) -> dict:
    """Compute EVoC hyperparameter ranges scaled to the dataset size.

    Problems with static ranges:
    - min_cluster_size (3-25) is too small for 20K+ chunks → fragments
    - noise_level (0.1-0.6) allows extreme values → 45%+ outliers
    - min_samples too high for small datasets, too low for large ones

    Scaling rules:
    - min_cluster_size: 5 to max(25, n/100), capped at 80
    - noise_level: tighter range centered on 0.25 (good for legal text)
    - n_neighbors: 5 to min(50, sqrt(n))
    - min_samples: 1 to max(3, n/500)
    """
    import math
    return {
        "base_min_cluster_size": (5, min(80, max(25, n_items // 100))),
        "noise_level": (0.15, 0.40),
        "n_neighbors": (5, min(50, max(15, int(math.sqrt(n_items))))),
        "min_samples": (1, max(3, min(10, n_items // 500))),
    }
...
def _select_best_layer(
    ...
    for li, layer in enumerate(layers):
        ...
        if prefer_granularity:
            ...
            composite = (
                scores.silhouette_score * 0.20
                + scores.average_cosine_similarity * 0.20
                + (1.0 - outlier_ratio) * 0.35
                + granularity_bonus * 0.25
            )
        else:
            composite = (
                scores.silhouette_score * 0.30
                + scores.average_cosine_similarity * 0.30
                + (1.0 - outlier_ratio) * 0.40
            )
